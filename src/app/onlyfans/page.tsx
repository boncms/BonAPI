...
export default function OnlyfansPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [onlyfans, mostViewed, random] = await Promise.all([
          dbService.getVideosOnlyfans(1000),
  ...
  const totalPages = Math.ceil(onlyfansVideos.length / itemsPerPage)
  const startIndex = (page - 1) * itemsPerPage
  const endIndex = startIndex + itemsPerPage
  const currentVideos = onlyfansVideos.slice(startIndex, endIndex)
          ...
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3">
                <Image src="/icons/onlyfans.png" alt="Onlyfans Premium" width={48} height={48} className="w-12 h-12" />
                <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white">Onlyfans Premium</h2>
              </div>
                <p className="text-dark-300">Exclusive content from top creators</p>
              </div>
